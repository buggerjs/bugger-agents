...
function evaluate(params) {
  ...
  if (returnByValue) {
    return this.client.evalSimple(expression, callFrameId)
      .then(function(value) {
        return {
          result: { value: value },
          wasThrown: false
  ...
  return {
    result: { type: 'string', value: 'Not implemented' },
    wasThrown: true
  };
};
...
function callFunctionOn(params) {
  ...
  if (objectId.indexOf('scope-handle:') !== 0) {
    throw new Error('Not implemented (scope handle only)');
  }
