...
DebuggerAgent.prototype._refreshScripts =
...
function setScriptSource(params) {
  return this.client.setScriptSource(params.scriptId, params.scriptSource);
};
...
function getScriptSource(params) {
  ...
  return this.client.getScriptsWithSource([ scriptId ])
    .then(function(scripts) {
      if (!scripts.length) {
        throw new Error('Script not found: ' + scriptId);
      }
      return {
        scriptSource: scripts[0].sourceCode
      };
    });
};
...
function getFunctionDetails(params) {
  ...
  if (functionId.indexOf('mirror:') === 0) {
    return this._getFunctionDetailsByMirror(functionId);
  } else if (functionId.indexOf('scope-handle:') === 0) {
    return this._getFunctionDetailsByHandle(functionId);
  } else {
    throw new Error('Not implemented: ' + functionId);
  }

};

DebuggerAgent.prototype._getFunctionDetailsByHandle =
function _getFunctionDetailsByHandle(functionId) {
  return this.client.lookupFunctionDetails(functionId)
    .then(function(details) { return { details: details }; });
};

DebuggerAgent.prototype._getFunctionDetailsByMirror =
function _getFunctionDetailsByMirror(functionId) {
  var expression = '__bugger__.probes.Runtime.getByObjectId(' + JSON.stringify(functionId) + ')';
  return this.client.lookupFunctionDetailsOfExpression(expression)
    .then(function(details) { return { details: details }; });
};
