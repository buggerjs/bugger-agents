...
function unlessBuggerPatched(fn, obj, additionalArgs) {
  if (!isBuggerPatched(obj)) {
    markBuggerPatched(obj);
    fn.apply(null, [ obj ].concat(additionalArgs));
  }
}

probe.enable = function enable() {
  // Setup http/https listeners
  unlessBuggerPatched(_patchLib, http, 'http:');
};

function _patchLib(lib, defaultProtocol) {
  var originalRequest = lib.request;

  function request(options, onResponse) {
    var protocol = options.protocol || (options._defaultAgent && options._defaultAgent.protocol) || defaultProtocol;
    var documentURL, splitPath;
    if (typeof options === 'string') {
      documentURL = options;
      options = Url.parse(options);
    } else {
      options.protocol = protocol;
      if (typeof options.path === 'string' && options.pathname === undefined) {
        splitPath = options.path.split('?');
        options.pathname = splitPath[0];
        options.search = splitPath[1];
      }
