...
probe.enable = function enable() {
  ...
  unlessBuggerPatched(_patchLib, http);
};
...
function _patchLib(lib) {
  ...
  function request(options, onResponse) {
    ...
    function onSocket(socket) {
      ...
      if (socket.ssl) {
        var originalHandshakeStart = socket.ssl.onhandshakestart;
        socket.ssl.onhandshakestart = function onTlsHandshakeStart() {
          trackTime('sslStart');
          return originalHandshakeStart.apply(this, arguments);
        };
      }
