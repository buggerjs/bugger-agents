...
function getResponseBody(params) {
  var chunks = this._responseBodyCache[params.requestId] || [];
  return {
...
NetworkAgent.prototype._onDataReceived =
function _onDataReceived(params) {
  if (!params.chunkBase64) return;

  var cache = this._responseBodyCache;
  var chunks = cache[params.requestId] = cache[params.requestId] || [];

  var chunk = new Buffer(params.chunkBase64, 'base64');
  delete params.chunkBase64;
  chunks.push(chunk);
};
