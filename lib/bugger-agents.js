...
function BuggerAgents(debugClient, child) {
  ...
  this._forwardEvent = function(debugEvent) {
    ...
    var match = debugEvent.method.match(/^(.+)\._buggerProbeResponse$/);
    if (match) {
      this[match[1]]._handleBuggerProbeResponse(debugEvent.params);
    }
...
BuggerAgents.prototype.injectFoundation = function injectFoundation() {
  ...
  var baseArgs = [
    sendEventInit,
    JSON.stringify(DEFAULT_DEFINES),
    JSON.stringify([ MODULES_PATH ]),
  ];
...
BuggerAgents.prototype.getMeta = function getMeta() {
  return this._child && this._child._meta;
};
